...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--cluster', required=True, type=int, help='ProxyCluster ID')
    ...
    def handle(self, *args, **options):
        ...
        if not https_servers:
            self.stdout.write(self.style.WARNING("No HTTPS server blocks found. Nothing to import."))
            return
        ...
        for _, sb in https_servers:
            ...
            if cert_path and key_path:
                ...
                if not dry:
                    ...
                    self.stdout.write(f"  {prefix}Would create SSL cert '{ssl_cert_name}' "
                                      f"({cert_path})")
                break
        ...
        for listen_addr, sb in https_servers:
            ...
            if sb.get('client_max_body_size'):
                custom_parts.append(f"client_max_body_size {sb.get('client_max_body_size')};")
            custom_directives = '\n'.join(custom_parts)
            ...
            for loc_block in sb.children:
                ...
                if try_files and not proxy_pass_url:
                    self.stdout.write(f"    {prefix}Location {path} → static (try_files)")
                ...
                upstream = None
                upstream_addr = re.sub(r'^https?://', '', proxy_pass_url.strip()).rstrip('/')
                is_local = upstream_addr.startswith('127.') or upstream_addr.startswith('10.0.')
                    ...
                    if upstream_name not in upstream_cache:
                        if not dry:
                            ...
                            if ucreated:
                                ...
                                self.stdout.write(f"    Upstream '{upstream_name}' → {upstream_addr} [created]")
                            else:
                                self.stdout.write(f"    Upstream '{upstream_name}' [exists]")
                            upstream_cache[upstream_name] = upstream
                        else:
                            self.stdout.write(f"    {prefix}Would create upstream '{upstream_name}' → {upstream_addr}")
                ...
                if request_buffering:
                    custom_loc_parts.append(f'proxy_request_buffering {request_buffering};')
                ...
                if not dry and vhost:
                    ...
                    self.stdout.write(f"    {prefix}Would create location {path} → {upstream_addr}")
            ...
            for loc_block in sb.children:
                ...
                if not dry and vhost:
                    ...
                    self.stdout.write(f"    {prefix}Would create location {path} → alias {alias}")
